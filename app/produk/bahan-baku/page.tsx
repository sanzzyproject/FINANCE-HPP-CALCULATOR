...
import { addData, getAllData, deleteData, updateData, Material } from '@/lib/db';
...
export default function BahanBakuPage() {
  ...
  const loadData = async () => {
    const data = await getAllData('materials');
    setMaterials(data);
  };
  ...
  const onSubmit = async (data: FormData) => {
    if (editingId) {
      await updateData('materials', { 
        id: editingId, 
        ...data,
      });
    } else {
      await addData('materials', { 
        ...data,
      });
    }
  ...
  const handleEdit = (material: Material) => {
    ...
    setValue('pricePerUnit', material.pricePerUnit);
          ...
          <table className="w-full text-sm text-left text-slate-500">
            <thead className="text-xs text-slate-700 uppercase bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-4 text-right">Harga per Satuan</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Harga per Satuan (Rp)</label>
                <input
                  type="number"
